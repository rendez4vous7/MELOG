...
* Linear Mode에서는 연속된 시간 셀(Time Cell)을 세미콜론(`;`)으로 구분할 수 있다.
...
### 4.10 시간 셀 구분자 (Time Cell Delimiter)

Linear Mode에서 세미콜론(`;`)은 연속된 시간 셀(Time Cell)을 구분한다.
하나의 Time Cell은 Graphical Mode의 정간보 한 칸에 대응하며, 하나의 이산 시간 프레임(Δt)을 의미한다.
...
위 예시는 3개의 연속된 Time Cell로 구성되며, 마지막 `||`는 문장의 종료를 의미한다.
