...
#       self.draw_demo()
    ...
    def render(self, tokens):
        ...
        for token in tokens:
            ...
            if isinstance(token, MelogCell):

                x = left + token.time * cell_width + 15

                for node in token.nodes:
                    y = top + (node.string - 1) * row_height + 5

                    self.draw_note(x, y, node)

    def draw_note(self, x, y, node):

        text = self.scene.addText(str(node.fret))
        text.setDefaultTextColor(QColor("white"))
        text.setFont(QFont("Consolas", 18))
        text.setPos(x, y)
...
class MelogIDE(QMainWindow):
    ...
    def change_domain(self, domain):
        ...
        self.converter = MelogConverter(domain)
        self.show_status(
            f"Domain switched to {domain}"
        )
    ...
    def create_center(self):
        ...
        self.editor.setPlainText(
            "(ROBOTICS)\n"
            "[#][F>] 1'1 2:1'1^ 3'30 4'1 6:1'7_ ;\n1'2 2:1'1^ 3'45 4'1 6:1'7_ ;\n1'3 2:1'1^ 3'60 4'1 6:1'7_ ;\n1'4 2:1'1^ 3'90 4'9 6:1'1_ ||"
        )
            ...
            for token in tokens:
                ...
                if isinstance(token, MelogCell):
                    ...
                    for node in token.nodes:
                        ...
                        cell_phonology.append(phonetic)

                    current_line.append(
                        "/" + "-".join(cell_phonology) + "/"
                    )

                elif isinstance(token, dict) and token.get("type") == "END":
