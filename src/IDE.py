...
class MelogGraphicEngine(QGraphicsView):
    ...
    def render(self, tokens):
        ...
        for token in tokens:
            ...
            if isinstance(token, MelogNode):
                string_num = token.string
                fret = token.fret

                x = left + time_index * cell_width + 15
                y = top + (string_num - 1) * row_height + 5

                self.draw_note(x,y,token)
        ...
        try:
            ...
            for token in tokens:
                ...
                if isinstance(token, MelogNode):
                    ...
                    phonology_lines.append(phonetic)

            phonology_text = "/" + "-".join(phonology_lines) + "/"
